...
export default async function AdminDashboard() {
  ...
  const stats = [
    ...
    { label: "Downloads", value: "—", href: "/admin/content/downloads", icon: <Download className="w-5 h-5" /> },
  ];
  ...
  const quickActions = [
    ...
    { label: "Add notification", desc: "Post a career, tender, or announcement", href: "/admin/content/notifications/new", icon: <Plus className="w-5 h-5" /> },
    { label: "Add event", desc: "Publish a new event or workshop", href: "/admin/content/events/new", icon: <Calendar className="w-5 h-5" /> },
    { label: "Add download", desc: "Upload a PDF or document", href: "/admin/content/downloads/new", icon: <Upload className="w-5 h-5" /> },
    { label: "Edit pages", desc: "Update homepage, about, contact text", href: "/admin/pages", icon: <FileEdit className="w-5 h-5" /> },
  ...
  return (
    <main className="p-8 max-w-5xl">
      ...
      {/* Quick actions */}
      <section aria-labelledby="actions-h" className="mb-10">
      ...
      {/* Site overview shortcut */}
      <section aria-labelledby="overview-h">
        <h2 id="overview-h" className="text-xs font-semibold uppercase tracking-widest mb-4" style={{ color: "#5a7c20" }}>
          Site overview
        </h2>
        <Link
          href="/admin/site-map"
          className="group bg-white rounded-xl border flex items-center gap-4 p-5 hover:shadow-sm transition-shadow"
          style={{ borderColor: "#e8f0e0" }}
        >
          <span
            className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0"
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold" style={{ color: "#1c2e06" }}>Browse the site map</p>
            <p className="text-xs mt-0.5" style={{ color: "#7a8e6a" }}>
              See all 43 public pages — click any row to edit or view it live
            </p>
          </div>
          <ArrowRight
            className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity shrink-0"
